...
def test_case(model, test_data, case_idx):
    ...
    # Reference data has 4 significant figures (e.g. 0.1255E+15), giving ~5e-4 relative precision
    TOL = 5e-4

    # Check temperature
    if ref_temp != 0.0:
        assert t[1] == pytest.approx(ref_temp, rel=TOL), (
    ...
    for i in range(10):
        ...
        if abs(ref) < 1e-35:
            ...
            assert got == pytest.approx(ref, rel=TOL), (
